// skills/audit/scripts/public/js/views/project-round.mjs
...
export async function renderProjectRound(container, params) {
  const projectName = params[0];
  const roundName = params[1];
  if (!projectName || !roundName) { location.hash = "#/home"; return; }
  ...
  let summary;
  try {
    summary = await api.getProjectRoundSummary(projectName, roundName);
  } catch (e) {
    container.innerHTML = `
      <div class="empty-state">
        ...
        <a href="#/project/${encodeURIComponent(projectName)}" class="btn btn-primary">${icon("arrowLeft", 14)} Back to Project</a>
      </div>`;
    return;
  }
  ...
  setBreadcrumb([
    ...
    { label: projectName, href: `#/project/${encodeURIComponent(projectName)}` },
    { label: roundName },
  ]);
  ...
  container.innerHTML = `
    ...
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-sm text-muted mt-1">${escapeHtml(roundName)} &middot; Aggregated across ${files.length} file${files.length !== 1 ? "s" : ""}</p>
      </div>
      <a href="#/project/${encodeURIComponent(projectName)}" class="btn btn-ghost">
        ${icon("arrowLeft", 14)} Back to Project
      </a>
    </div>
    ...
    ${files.length === 0 ? `
      <div class="empty-state">
        ...
        <p>This round has no completed reviews.</p>
      </div>
    ` : `
      <div class="card">
